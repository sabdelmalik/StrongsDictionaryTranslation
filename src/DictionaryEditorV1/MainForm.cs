...
namespace DictionaryEditorV1
{
    public partial class MainForm : Form
    {
        ...
        {
            string dbName = "StrongsDictionaryV2";
        ...
        public bool GetDictionaryTranslation(int language, string strongsNumber, string dStrong, bool trace, LexiconEntry entry)
        {
            ...
            try
            {
                ...
                command.CommandText =
                   "SELECT  " +
                   "xlation.translated_gloss," +
                   "xlation.translated_text," +
                   "xlation.transliteration," +
                   "xlation.update_date," +
                   "xlation.updater_name," +
                   "status.status," +
                   "google.name" +
                   " FROM public.\"strongs_numbers\" strongs" +
                   " INNER JOIN public.\"translation\" xlation" +
                       " ON strongs.strong_id = xlation.strong_id" +
                   " INNER JOIN public.\"translation_status\" status" +
                       " ON status.id = xlation.status_id" +
                    " INNER JOIN public.\"google_sheet\" google" +
                       " ON google.sheet_id = xlation.sheet_id" +
                    string.Format(" WHERE google.translation_language={0} AND strongs.strongs_number='{1}' AND strongs.d_strong='{2}';",
                       language,
                       strongsNumber,
                       dStrong.Trim());
                ...
                if (entry == null)
                {
                    ...
                    tbLastUpdated.Text = string.Empty;
                    tbUpdater.Text = string.Empty;
                    tbUpdateStatus.Text = string.Empty;
                    tbSheetName.Text = string.Empty;
                }
                ...
                while (reader.Read())
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            tbTranslatedLong.Text = (string)reader[1];
                            tbArabicTxLt.Text = (string)reader[2];
                            tbLastUpdated.Text = ((DateTime)reader[3]).ToString("yyyy-MM-dd HH:mm:ss");
                            tbUpdater.Text = (string)reader[4];
                            tbUpdateStatus.Text = (string)reader[5];
                            tbSheetName.Text = (string)reader[6];
                        }
                    }
