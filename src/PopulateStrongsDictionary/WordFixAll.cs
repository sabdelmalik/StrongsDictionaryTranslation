...
{
    internal class WordFixAll
    {
        Dictionary<string, WordEntryAll> dictionaryWords = new Dictionary<string, WordEntryAll>();
        Dictionary<string, WordEntryAll> dictionaryMatchGoogle = new Dictionary<string, WordEntryAll>();
        Dictionary<string, WordEntryAll> dictionarySingleWord = new Dictionary<string, WordEntryAll>();
        Dictionary<string, WordEntryAll> dictionaryOther = new Dictionary<string, WordEntryAll>();

        SortedDictionary<string, WordEntryAll> dictionaryCorrected = new SortedDictionary<string, WordEntryAll>();
        ...
        public WordFixAll(MainForm mainForm) 
        {
        ...
        public bool Excute(NpgsqlDataSource dataSource)
        {
            ...
            using (StreamWriter sw = new StreamWriter(reportPath))
            {
                ...
                {
                    WordEntryAll we = dictionaryCorrected[strongs];
        ...
        private bool ApplyCorrections(NpgsqlDataSource dataSource)
        {
            ...
            try
            {
                ...
                foreach (string strongNumber in dictionaryCorrected.Keys)
                {
                    ...
                    WordEntryAll entry = dictionaryCorrected[strongNumber];
        ...
        private bool BuildCorrectionDictionaries(NpgsqlDataSource dataSource)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    string step_united_reason = reader[4].ToString();
                    string step_type = (string)reader[5];
                    string transliteration = (string)reader[6];
                    string translated_word = (string)reader[7];
                    string word = (string)reader[8];
                    string longText = (string)reader[9];
                    ...
                    {
                        WordEntryAll wordEntry = new WordEntryAll(original_word, english_translation, step_united_reason, step_type, transliteration, translated_word, longText);
        ...
        private void DoCorrections()
        {
            ...
            {
                WordEntryAll wordEntry = dictionaryWords[strongs];
        ...
        private string GetTrimmedWord(string araWord, string hebOrGrkWord, bool heb)
        {
           ...
           if (!string.IsNullOrEmpty(araWord) && !string.IsNullOrEmpty(hebOrGrkWord))
            {
                try
                {
                    if (heb)
    ...
    class WordEntryAll
    {
            ...
            string longText)
        {
            ...
            this.StepUnitedReason = step_united_reason;
        }
        ...
        public string StepUnitedReason { get; }
